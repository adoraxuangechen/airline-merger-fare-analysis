...
This supplement accompanies Assessing Fare Changes After the Delta–Northwest Merger. It records the data construction, implementation checks, additional diagnostics, and reconciliation behind the writing sample. The main relative fare contrast is -5.72%. The pre-period evidence limits its causal interpretation; the material below makes the numerical analysis reproducible without treating reproducibility as identification.
...
The baseline coefficient is -0.0589, with clustered standard error 0.0189 and p = 0.0020. Its transformed contrast is -5.72%, with 95% interval [-9.16%, -2.15%]. Route clustering allows dependence within an airport pair but does not allow unrestricted correlation across pairs sharing airports or airline networks. The interval does not incorporate bias from an unsuitable comparison group.
...
The joint pre-completion test is F(14, 260) = 8.925, p = 7.09e-16. A separate model restricted to 2005–2007, omitting 2007Q4, gives F(11, 260) = 7.456, p = 4.11e-11. Both concern changes in observed relative gaps, potentially including differential seasonality. The second test avoids an announcement-period reference.
...
This procedure selects only 28 distinct comparison routes, with maximum reuse of 30. The matched panel has 4,416 cells and 184 routes. Its fare contrast is -4.55%, but the pre-completion event coefficients remain jointly significant: F(14, 183) = 12.278, p < 0.001. Reuse concentration and remaining pre-period differences make this an informative diagnostic, not a solution to the comparison problem. The inherited distance feature also retains the source zero sentinel, which further limits its interpretation as a validated match on distance.
